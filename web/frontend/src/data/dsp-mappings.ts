...
export interface DspMapping {
  [key: string]: string;
}

export const dspIdToDisplayName: DspMapping = {
  // Noise Gates
...
// Function to get display name from DSP ID
export function getDisplayName(dspId: string): string {
  return dspIdToDisplayName[dspId] || dspId;
}

// Function to check if DSP ID is recognized
export function isKnownDspId(dspId: string): boolean {
  return dspId in dspIdToDisplayName;
}